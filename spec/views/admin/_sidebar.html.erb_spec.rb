...
RSpec.describe 'admin/_sidebar' do
  ...
  describe 'vocabularies link' do
    it 'renders for authorized users' do
      allow(view.controller.current_ability).to receive(:can?).with(:read, Vocabulary).and_return(true)
      render
      expect(rendered).to have_link(href: vocabularies_path)
    end

    it 'is hidden from unauthorized users' do
      allow(view.controller.current_ability).to receive(:can?).with(:read, Vocabulary).and_return(false)
      render
      expect(rendered).not_to have_link(href: vocabularies_path)
    end
  end
